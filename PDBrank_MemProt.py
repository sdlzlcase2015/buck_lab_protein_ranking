...
count=-1;m=-1;n=-1;indicator=0; rep=0;q=0;mem=0
for line in fp: 
   ...
   if "</recommendedName>" in line:
       q=0;rep=0
   if (q==0 and rep==0) and ( "Multi-pass" in line or  "multi-pass" in line  or  "Single-pass" in line or  "single-pass" in line):
       mem=1
   if "PDBsum" in line and mem==1:
...
with open('ProteinRankMem.csv', mode='w') as csv_file:
